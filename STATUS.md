...
- Gerada uma `APP_KEY` real e criado `equipe/app/.env.production.ready` — um `.env` de produção **completo e pronto**, nunca commitado (gitignored), que o usuário só precisa subir e renomear para `.env` no servidor. Validado temporariamente como `.env` ativo localmente (rodando a suíte completa + servindo a aplicação de verdade) antes de restaurar o `.env` de desenvolvimento.
- **Bug de isolamento de teste encontrado e corrigido**: ativar o `.env` de produção (com `APP_URL=.../visitas`) quebrou 40 testes com erro 405 — o cliente de teste do Laravel monta URLs de teste concatenando `APP_URL` + path literalmente (sem um Apache de verdade pra stripar o prefixo antes). Não é um bug de produção (Apache faz esse strip de verdade), mas expôs que a suíte de testes dependia implicitamente do `APP_URL` do `.env` ativo. Corrigido adicionando `<env name="APP_URL" value="http://localhost"/>` em `phpunit.xml`, isolando os testes de qualquer valor real de produção.
- `equipe/data/neurologia.sqlite3` local recriado do zero (`migrate:fresh --seed`) e verificado (`PRAGMA integrity_check` → `ok`) — esse arquivo, exatamente como está, é o que o usuário sobe para o servidor: já vem com o admin inicial (`admin`/`senha@1234`), especialidades e planos de exemplo.
- Build de produção gerado (`VITE_BASE_PATH=/visitas/`) e copiado para `public_html/visitas/` — pronto para upload.
- `DEPLOYMENT_HOSTGATOR.md` reescrito do zero num formato "arrasta e solta": duas pastas para subir (`equipe/` e `public_html/visitas/`), um único passo manual de atenção (renomear `.env.production.ready` para `.env` no servidor), e uma seção de referência técnica (comandos Artisan/cron) recolhida para quando/se o usuário tiver acesso a terminal no futuro.
- Validado de ponta a ponta com o `.env` de produção final + banco final + build final juntos (via `vite preview` com o prefixo `/visitas/` real): login → troca de senha → dashboard → integrity_check — tudo funcionando.
